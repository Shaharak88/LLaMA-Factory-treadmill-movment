...
- Category distributions for all metadata parameters
...
Usage:
    python3 generate_enhanced_video_report.py METADATA_CSV_FILE

Example:
    python3 generate_enhanced_video_report.py data/_exp_20251201_172008_train_metadata_20251202_130454.csv
"""
...
<body>
    <div class="container">
        <header>
            ...
            <p style="font-size: 0.9em; margin-top: 10px;">
                ✅ With category distributions, warnings, and imbalance analysis
            </p>
        </header>
    ...
    <script>
        ...
        function renderQuickStats() {{
            ...
            const keyParams = ['texture', 'direction', 'speed', 'angle'];
            keyParams.forEach(param => {{
                if (stats.distributions[param]) {{
                    const data = stats.distributions[param];
                    html += `<li><strong>${{param.charAt(0).toUpperCase() + param.slice(1)}}:</strong> ${{data.unique_values}} unique values</li>`;
                }}
            }});
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog='Example: python3 generate_enhanced_video_report.py data/_exp_20251201_172008_train_metadata_20251202_130454.csv'
    )
    ...
    generate_html_report(metadata, stats, groups, missing_combos, heatmap_data, args.output)
